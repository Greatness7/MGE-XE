...
HRESULT _stdcall MGEProxyD3D::CreateDevice(UINT a, D3DDEVTYPE b, HWND c, DWORD d, D3DPRESENT_PARAMETERS8* e, IDirect3DDevice8** f) {
    ...
    // Override device parameters; Morrowind checks the modified parameters on return
    ...
    // Let the game manage focus and size changes. DXVK otherwise sends focus
    // restore window messages that can re-enter Morrowind's renderer restart.
    DWORD behaviorFlags = d | D3DCREATE_NOWINDOWCHANGES;

    IDirect3DDevice9* realDevice = NULL;
    HRESULT hr = realD3D->CreateDevice(a, b, c, behaviorFlags, &pp, &realDevice);
    ...
    const bool bc7Supported = SUCCEEDED(realD3D->CheckDeviceFormat(
        a, b, D3DFMT_X8R8G8B8, 0, D3DRTYPE_TEXTURE, MGE_D3DFMT_BC7));
    MWTextureLoader::setBC7TextureSupport(bc7Supported);
