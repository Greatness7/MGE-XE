...
impl WeatherEditorState {
    fn new(weather: &WeatherSet) -> Self {
        Self {
...
const WEATHER_SIZE: [f32; 2] = [524.0, 409.0];
...
impl GuiApp {
    ...
    pub(in crate::ui) fn open_weather_settings(&mut self) {
        self.ui.distant.weather = Some(WeatherEditorState::new(&self.settings.mge.distant_land.weather));
    }
        ...
        CentralPanel::default().frame(Frame::NONE.fill(style::APP_BG)).show(ui, |ui| {
            ...
            for (column, (title, tip)) in WEATHER_COLUMNS.iter().enumerate() {
                ...
                let caption = group_box(
                    ui,
                    at(WEATHER_GB_X[column], GB_Y, WEATHER_GB_W[column], GB_H),
                    title.as_ref(),
                    &font,
                );
            ...
            for (index, name) in WEATHER_NAMES.iter().enumerate() {
                ...
                for column in 0..WEATHER_COLUMNS.len() {
                    ...
                    let drag = DragValue::new(field(&mut state.draft[index]));
                    // Legacy `NumericUpDown` DecimalPlaces / Increment /
                    // Minimum / Maximum, column by column.
                    let drag = match column {
            ...
            if tooltip(
                ui.put(
                    at(WEATHER_BTN_X[0], BTN_Y, BTN_W, BTN_H),
            ...
            if tooltip(
                ui.put(
                    at(WEATHER_BTN_X[1], BTN_Y, BTN_W, BTN_H),
            ...
            if tooltip(
                ui.put(
                    at(WEATHER_BTN_X[2], BTN_Y, BTN_W, BTN_H),
