...
void MWBridge::InitStaticMemory() {
    // Morrowind + Tribunal + Bloodmoon v1.6.1820
...
void MWBridge::toggleRipples(BOOL enabled) {
    ...
    MWPatches::VirtualMemWriteAccessor vw((void*)addr, 4);
...
bool MWBridge::getLoadingBarLabel(std::string& out) {
    const auto findMenu = reinterpret_cast<void*(__cdecl*)(short)>(0x595370);
    void* loadingMenu = findMenu(*reinterpret_cast<const short*>(0x7d4238));
    if (!loadingMenu) {
        return false;
    }

    const auto findChildElement = reinterpret_cast<void*(__thiscall*)(void*, short)>(0x582de0);
    void* loadingLabel = findChildElement(loadingMenu, *reinterpret_cast<const short*>(0x7d4204));
    if (!loadingLabel) {
        return false;
    }

    const auto getText = reinterpret_cast<const char*(__thiscall*)(void*)>(0x580bb0);
...
void MWBridge::setLoadingBarLabel(const char* text) {
    const auto updateLoadingLabel = reinterpret_cast<void(__cdecl*)(const char*)>(0x5deef0);
...
void MWBridge::updateLoadingScreen(float percent) {
    const auto loadingScreenUpdate = reinterpret_cast<void(__cdecl*)(float)>(0x4c7d90);
...
void MWBridge::setUIScale(float scale) {
    ...
    MWPatches::VirtualMemWriteAccessor vw((void*)addr, sizeof(patch));
