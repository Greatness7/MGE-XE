...
IDirect3DSurface9* ProxyDevice::dsvSubstituteFrom = NULL;
IDirect3DSurface9* ProxyDevice::dsvSubstituteTo = NULL;
...
void ProxyDevice::setDepthStencilSubstitute(IDirect3DSurface9* from, IDirect3DSurface9* to) {
    dsvSubstituteFrom = from;
    dsvSubstituteTo = to;
}
...
HRESULT _stdcall ProxyDevice::SetRenderTarget(IDirect3DSurface8* a, IDirect3DSurface8* b) {
    ...
    if (b_real != NULL && b_real == dsvSubstituteFrom) {
        b_real = dsvSubstituteTo;
    }
