...
namespace api {
    ...
    bool MGEAPIv1::loadConfig() {
        ...
        bool success = Configuration.ReloadSettings();

        if (success) {
            if (DistantLand::isDistantLandLoaded
                && !DistantLand::staticsUploaded
                && (Configuration.MGEFlags & USE_DISTANT_STATICS)) {
                Configuration.MGEFlags &= ~USE_DISTANT_STATICS;
                LOG::logline("-- Ignoring runtime distant-statics enable: static resources were skipped; restart the renderer to upload them");
            }
    ...
    void MGEAPIv1::featureSetEnabled(RenderFeature feature, bool enable) {
        ...
        if (i >= 0 && i < featureToFlagMap.size()) {
            auto flag = featureToFlagMap[i];
            if (flag == USE_DISTANT_LAND
                && enable
                && !DistantLand::isDistantLandLoaded) {
                LOG::logline("-- Ignoring runtime distant-land enable: distant-land resources were not loaded at startup");
                return;
            }
            if (flag == USE_DISTANT_STATICS
                && enable
                && DistantLand::isDistantLandLoaded
                && !DistantLand::staticsUploaded) {
                LOG::logline("-- Ignoring runtime distant-statics enable: static resources were skipped; restart the renderer to upload them");
    ...
    void MGEAPIv2::saveScreenshot(const char* path, bool captureWithUI) {
        ...
        if (!DistantLand::canRenderDistantLand()) {
            return;
        }
        if (!path) {
            return;
