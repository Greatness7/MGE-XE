...
namespace IPC {
	...
	*/
	...
	private:
		...
		bool m_isRpcPending;
		LaunchState m_launchState;

		bool beginRpc(Command command);
		WakeReason waitForBootstrapCompletion();
	...
	public:
		...
		bool launchServer(const char* executable, const char* workingDirectory = nullptr);
		bool startServer(const char* executable, const char* workingDirectory = nullptr);
		void stopServer();
		WakeReason waitForBootstrapReady();
		WakeReason pollBootstrapReady();
		WakeReason pollRpcCompletion();
		bool isServerActive();
		LaunchState launchState() const { return m_launchState; }
		void markRuntimeActive() { m_launchState = RuntimeActive; }
		...
		*/
		bool getVisibleMeshes(
			...
			float nearStaticEnd,
			float farStaticEnd,
			VisibleSetSort sort = VisibleSetSort::None
		);
		...
		*/
		...
		/**
		* @brief Push live terrain horizon-culling configuration to the server and wait for it to apply.
		*
		* Blocking: the host may rebuild its terrain height field (when the sample spacing changes, or
		* when culling is enabled with no field loaded) before the next visibility query, so this returns
		* only once the host has applied the new configuration.
		*
		* @param config The horizon-culling parameters to apply.
		* @return Whether the RPC completed successfully.
		*/
		bool setHorizonConfig(const SetHorizonConfigParameters& config);

		/**
		* @brief Asynchronously close the current render frame for the adaptive horizon gate.
		*
		* Must be sent once per rendered frame while horizon culling is enabled, after all of that
		* frame's precise static visibility queries (main distant statics and any reflected statics),
		* so the adaptive gate's benefit signal is scoped to a single real frame instead of bleeding
		* into the next one.
		*
		* @return Whether the RPC was issued successfully.
		*/
		bool finishHorizonFrame();
