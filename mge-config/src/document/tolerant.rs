...
// `distantland/crates/job/src/tolerant.rs`; the caller-specific entry points intentionally differ.
...
) -> Result<Settings, serde_path_to_error::Error<toml_edit::de::Error>> {
    serde_path_to_error::deserialize(toml_edit::de::Deserializer::from(document.clone()))
}

/// Collects the paths the `Settings` schema does not claim, so callers can warn
/// about hand-edited keys without losing them.
pub(super) fn ignored_paths(document: &DocumentMut) -> Result<Vec<Vec<PathSegment>>, toml_edit::de::Error> {
    let mut ignored = Vec::new();
    serde_ignored::deserialize::<_, _, Settings>(toml_edit::de::Deserializer::from(document.clone()), |path| {
...
pub(super) fn error_segments(path: &serde_path_to_error::Path) -> Vec<PathSegment> {
    ...
    for segment in path {
        match segment {
            ...
            ErrorSegment::Enum { variant } => {
                segments.push(PathSegment::Key(variant.clone()));
            }
...
/// Removes a bad value. If it lives in a sequence, the smallest enclosing
/// element is removed so sibling entries remain usable.
pub(super) fn remove_bad_value(document: &mut DocumentMut, path: &[PathSegment]) -> bool {
    ...
    remove_path(document, &path[..end])
}

pub(super) fn remove_path(document: &mut DocumentMut, path: &[PathSegment]) -> bool {
    remove_from_table(document.as_table_mut(), path)
}
